...
Pilha inverterString(Pilha* pilha){
	if(pilhaVazia(pilha) == true) printf("A pilha está vazia!");
	else{
		No* aux = pilha->topo;
...
void palindromo(Pilha* pilha){
	if(pilhaVazia(pilha) == true) printf("A pilha está vazia!");
		...
		Pilha pilhaInvertida = inverterString(pilha);
		
		No* aux = pilha->topo;
		No* auxInvertida = pilhaInvertida.topo;
		
		while(aux != NULL && auxInvertida != NULL){
			if(aux->letra != auxInvertida->letra){
				printf("A String não é palindromo!");
				return;
			}
			else{
				aux = aux->proximo;
				auxInvertida = auxInvertida->proximo;
			}
		}
